...
<head>
    ...
    <title>Assessment Center - {{ $penilaian->nama }}</title>
...
<body class="cyber-gradient min-h-screen">
        ...
        <div class="container mx-auto px-6 py-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <h1 class="game-font text-2xl font-bold text-white">{{ $penilaian->nama }}</h1>
                    <div class="w-1 h-8 bg-blue-400 rounded-full"></div>
                    <span class="text-gray-300">{{ $penilaian->jenis_text }}</span>
                </div>
                <div class="text-right">
                    <div class="text-xl font-bold text-green-400 game-font" id="timer">
                        {{ $penilaian->durasi_menit }}:00
                    </div>
    ...
    <div class="container mx-auto px-6 py-8">
        <div class="bg-black/80 backdrop-blur-sm rounded-2xl p-8 cyber-border neon-glow">
            ...
            <div class="mb-8">
                ...
                <div class="bg-gray-900/50 rounded-xl p-6 cyber-border">
                    <p class="text-gray-300 leading-relaxed">
                        {{ $penilaian->petunjuk ?: 'Lorem Ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.' }}
                    </p>
                </div>
            </div>
                        ...
                        <div class="bg-gray-900/50 rounded-xl p-6 cyber-border min-h-32">
                            <div class="flex items-start space-x-3">
                                ...
                                <div class="text-gray-300 leading-relaxed">
                                    {{ $penilaian->konten ?: 'Deskripsi soal studi kasus akan ditampilkan di sini. Peserta diminta untuk menganalisis situasi yang diberikan dan memberikan jawaban yang komprehensif berdasarkan pemahaman dan pengalaman yang dimiliki.' }}
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            @elseif($penilaian->jenis === 'in_tray')
                ...
                <form id="inTrayForm" class="space-y-6">
                    ...
                    <div class="flex justify-between pt-6">
                        <button 
                            type="button" 
                            onclick="saveInTrayJawaban('draft')"
                            class="bg-gray-600 hover:bg-gray-700 text-white px-8 py-3 rounded-lg transition-colors duration-200 game-font neon-glow"
                        >
                            SIMPAN SEMENTARA
                        </button>
                        <button 
                            type="button" 
                            onclick="saveInTrayJawaban('final')"
                            class="bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white px-8 py-3 rounded-lg transition-all duration-200 game-font neon-glow"
                        >
                            SIMPAN FINAL
                        </button>
            ...
            @elseif($penilaian->jenis === 'roleplay')
                <!-- Role-Play Form -->
                <form id="roleplayForm" class="space-y-6">
                    <div>
                        ...
                        <div class="bg-gray-900/50 rounded-xl p-6 cyber-border min-h-32">
                            <div class="text-gray-300 leading-relaxed">
                                {{ $penilaian->konten ?: 'Instruksi untuk role-play akan ditampilkan di sini. Peserta diminta untuk memahami skenario yang diberikan dan mempersiapkan diri untuk melakukan role-play sesuai dengan instruksi yang ada.' }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-between pt-6">
                        <button 
                            type="button" 
                            onclick="saveCatatanRoleplay('draft')"
                            class="bg-gray-600 hover:bg-gray-700 text-white px-8 py-3 rounded-lg transition-colors duration-200 game-font neon-glow"
                        >
                            SIMPAN SEMENTARA
                        </button>
                        <button 
                            type="button" 
                            onclick="saveCatatanRoleplay('final')"
                            class="bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white px-8 py-3 rounded-lg transition-all duration-200 game-font neon-glow"
                        >
                            SIMPAN FINAL
                        </button>
            ...
            @elseif($penilaian->jenis === 'fgd')
                ...
                <form id="fgdForm" class="space-y-6">
                    <div>
                        ...
                        <div class="bg-gray-900/50 rounded-xl p-6 cyber-border min-h-32">
                            <div class="text-gray-300 leading-relaxed">
                                {{ $penilaian->konten ?: 'Instruksi untuk Forum Group Discussion (FGD) akan ditampilkan di sini. Peserta diminta untuk memahami topik yang akan didiskusikan dan mempersiapkan diri untuk berpartisipasi aktif dalam diskusi kelompok.' }}
                            </div>
                        </div>
                    </div>
    ...
    <script>
        // Timer countdown
        var remainingTime = {{ $penilaian->getRemainingTime() ?? 0 }};
        ...
        function saveJawaban(status) {
            ...
            fetch('{{ route("penilaian.studi-kasus.save", $penilaian->id) }}', {
                method: 'POST',
                headers: {
        ...
        function saveCatatanRoleplay(status) {
            ...
            fetch('{{ route("penilaian.roleplay.save", $penilaian->id) }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        function saveCatatanFgd(status) {
            var catatan = document.getElementById('catatanFgd').value;
            if (!catatan.trim()) {
                alert('Mohon isi catatan terlebih dahulu.');
                return;
            }

            fetch('{{ route("penilaian.fgd.save", $penilaian->id) }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'

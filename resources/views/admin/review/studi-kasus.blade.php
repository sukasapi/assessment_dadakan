...
@include('admin.partials.page-header', [
    'title' => 'Review Jawaban Studi Kasus',
    ...
    'actions' => '<a href="' . route('admin.progress.index') . '" class="admin-btn-secondary">Kembali ke Progress</a>',
        ...
        <div class="admin-card p-6 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    <h3 class="text-lg font-medium text-primary">Total Jawaban</h3>
            ...
            <div class="admin-card">
                <div class="px-6 py-4 border-b border-gray-200">
                    <div class="flex justify-between items-center">
                        <div>
                            <h3 class="text-lg font-medium text-primary">{{ $jawaban->peserta->nama_lengkap }}</h3>
                            <p class="text-sm text-tertiary">{{ $jawaban->peserta->email }}</p>
                            <p class="text-sm text-tertiary">{{ $jawaban->peserta->jabatan_saat_ini }} - {{ $jawaban->peserta->instansi }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-sm text-tertiary">Submit: {{ \Carbon\Carbon::parse($jawaban->waktu_simpan)->format('d/m/Y H:i') }}</p>
                            @if($jawaban->status)
                ...
                <div class="p-6">
                    <!-- Case Study Content -->
                    <div class="mb-6">
                        <h4 class="font-medium text-primary mb-3">Studi Kasus:</h4>
                        <div class="bg-gray-50 rounded-lg p-4">
                            <p class="text-primary">{{ $jawaban->konten_studi_kasus ?: 'Tidak ada konten studi kasus' }}</p>
                        </div>
                    </div>

                    <!-- Participant's Answer -->
                    <div class="mb-6">
                        <h4 class="font-medium text-primary mb-3">Jawaban Peserta:</h4>
                        <div class="bg-blue-50 rounded-lg p-4">
                            <p class="text-primary">{{ $jawaban->jawaban_peserta ?: 'Belum ada jawaban' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                        <h4 class="font-medium text-primary mb-3">Catatan Tambahan:</h4>
                        <div class="bg-yellow-50 rounded-lg p-4">
                            <p class="text-primary">{{ $jawaban->catatan_tambahan }}</p>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <h4 class="font-medium text-primary mb-2">Waktu Mulai:</h4>
                            <p class="text-sm text-tertiary">
                                {{ $jawaban->waktu_mulai ? \Carbon\Carbon::parse($jawaban->waktu_mulai)->format('d/m/Y H:i:s') : 'Belum dimulai' }}
                            </p>
                        </div>
                        <div>
                            <h4 class="font-medium text-primary mb-2">Waktu Selesai:</h4>
                            <p class="text-sm text-tertiary">
                        ...
                        <div>
                            <h4 class="font-medium text-primary mb-2">Durasi:</h4>
                            <p class="text-sm text-tertiary">
                        ...
                        @endif
                        <div>
                            <h4 class="font-medium text-primary mb-2">Status:</h4>
                            <p class="text-sm text-tertiary">
        ...
        <div class="text-center py-12">
            ...
            <p class="mt-1 text-sm text-tertiary">Jawaban studi kasus akan muncul di sini setelah peserta submit</p>
        </div>
        @endif
